...
All reads/writes go through `db/promptRepository.ts` functions (`createPrompt`, `updatePrompt`, `deletePrompt`, `getPromptById`, `listAll`, `listFavorites`, `listRecentlyUsed`, `searchPrompts`, `listCategories`, `listTags`, `toggleFavorite`, `recordUsage`, `bulkInsert`, `countAll`, `getAppMeta`/`setAppMeta`/`incrementAppMeta`, `hasEverExported`, `markExported`). Screens and hooks never touch SQL directly.
...
Routes under `app/`: `_layout.tsx` (DB init + migrations + seed), `(tabs)/_layout.tsx` (3-tab shell: All Prompts / Favorites / Recently Used), `(tabs)/index.tsx` (All), `(tabs)/favorites.tsx`, `(tabs)/recent.tsx`, `prompt/new.tsx`, `prompt/[id]/index.tsx` (Detail), `prompt/[id]/edit.tsx`, `prompt/[id]/fill.tsx`, `settings/index.tsx`, `settings/import-preview.tsx`. Shared components in `components/` (SearchBar, PromptCard, PromptList, SectionHeader, VariableField, PreviewPane, EmptyState, BackupReminderBanner, ImportPreviewList, ConfirmDialog, Toast, AdBanner, AdGateHost, Tooltip, FeatureBadge, DisabledStateHelper, CategoryFilter).
...
`react-native-google-mobile-ads` (v16) with the Expo config plugin. App IDs in `app.json` use the **production** AdMob account for Android (`ca-app-pub-6917313063209470...`, package `com.hoangweb.prompttemplate` with `google-services.json` wired via `android.googleServicesFile`). Unit IDs are selected by the `TEST_ADS` flag in `lib/config.ts` (default `false` — production is the shipped build; set to `true` during development/testing): when true, all four formats (banner/interstitial/rewarded/app open) use Google's official **test** IDs on Android and iOS and the UMP consent gate is skipped (test ads never serve real traffic); when false, Android uses the production IDs and iOS still uses test IDs as placeholders until an iOS release is prepared. Banner (ANCHORED_ADAPTIVE) pinned to the bottom of Home, Settings and Detail; interstitial, rewarded and app-open ads each managed by a singleton that preloads in the background. All ad requests are gated on the UMP consent flow (`lib/ads.ts` `initializeAds`/`canRequestAds`) when not in test mode: the SDK and ad preloads only start once `canRequestAds` is true (`delayAppMeasurementInit` + consent proguard rule configured), and Settings shows a privacy-options re-review row when the UMP flow reports it required.
...
- Non-personalized ads only (`requestNonPersonalizedAdsOnly: true`); production requests are still gated on the UMP consent flow (EEA/UK), while test mode skips the gate entirely.
...
- Frequency gate (every 10 copies) keeps the core loop intact; ads only show if already loaded, never delaying the copy.
...
- [Test vs production IDs] → Mitigation: single `TEST_ADS` flag in `lib/config.ts` switches all four formats; shipped build defaults to production, dev/test flips to Google test IDs (never produce revenue, clearly visible as test).
