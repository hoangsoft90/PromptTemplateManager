...
| Bài học/lỗi lặp lại đã đúc kết | AgentMemory (`/lessons`, `/crystals`) |
...
**Prompt Template Manager** — app **Expo / React Native** (managed workflow) chứa thư viện prompt template cá nhân, 100% on-device, không cloud/backend/account. Core loop: **Search → Fill (nếu có biến) → Copy**. Vai trò agent: full-stack mobile dev (TypeScript/Expo), kèm build Android APK (gradle local + GitHub Actions), thiết kế qua OpenSpec (`openspec/`). Tech stack: expo-router · expo-sqlite · expo-clipboard · expo-haptics · expo-document-picker · expo-file-system · expo-sharing · expo-crypto · react-native-google-mobile-ads v16 · Jest.

## Initial Setup & Navigation
`context.md`/`working.md`/`operating_rules.md` (ở project root) đã được đọc tự động ở "SESSION START" — KHÔNG đọc lại ở đây, tránh tốn token 2 lần. Bước navigation RIÊNG:
- **Khi cần hiểu thiết kế/chạy task lớn**: đọc `openspec/` trước — `openspec/changes/prompt-template-manager-mvp/` (MVP, design D1–D9) và `openspec/changes/add-in-app-guidance-onboarding/` (onboarding). Source of truth gốc: `.plan/plan1_implementation_spec.md`.
- **Khi sửa data layer**: bề mặt truy cập duy nhất là `db/promptRepository.ts` (đa nền tảng `.shared.ts`/`.web.ts`/`.sqlite.ts`) — không chạm SQL trực tiếp từ screen/hook.
- **Khi sửa native/Android**: không sửa `android/` trực tiếp (bị `expo prebuild` regenerate) — phải qua config plugin trong `plugins/` và đăng ký trong `app.json`.
- **Khi cần test**: `npx jest` (full suite), `npx tsc --noEmit`, `openspec validate --all` nếu đụng spec.

## Critical Rules (Must Follow)
<!-- Các rule RIÊNG của project (chi tiết đầy đủ trong operating_rules.md — chỉ tóm tắt điểm chốt ở đây) -->
1. **`TEST_ADS` phải về `false` trước khi publish** (`lib/config.ts`) + thay 4 iOS placeholder unit IDs bằng production IDs — không release với test ads.
2. **Mọi non-DB pref qua `app_meta`** (`getAppMeta`/`setAppMeta`) — không thêm AsyncStorage/MMKV.
3. **Sửa native qua config plugin** (`plugins/`), không sửa trực tiếp `android/`.
4. **Giữ `targetSdkVersion: 36`** (Google Play yêu cầu API 36 từ 31/8/2026) — không hạ xuống.
5. **Onboarding components** (`Tooltip`/`DisabledStateHelper`) phải passthrough khi không có `OnboardingProvider` — không crash screen standalone.
...
## Workflow
- **Bug fixing**: tái hiện lỗi → trace qua `lib/` + `db/` + screen liên quan (ưu tiên codebase-memory-mcp/cocoindex) → viết test fail → fix → chạy test liên quan. UI/ads bug test trên phone cần build lại (`expo run:android`), vì APK cũ không phản ánh thay đổi native.
- **Feature add**: 
  - Kiểm tra xem có nên tạo/cập nhật OpenSpec change không (behavior thay đổi → có).
  - Viết code theo convention project (đọc file lân cận trước), test kèm theo.
  - Update `working.md` sau khi làm xong.
  - Chạy `tsc --noEmit` + Jest (tối thiểu test liên quan) trước khi báo cáo cho user.
  - Nếu đụng spec/behavior: cập nhật openspec + `openspec validate`.
...
- **Branch/Commit convention:** Conventional Commits (`feat:`, `fix:`, `chore:`, `docs:`, `test:`). Branch trực tiếp trên `main` cho thay đổi nhỏ; với OpenSpec change lớn tạo branch riêng (`feat/<change-name>`) và merge khi archive xong. Repo đã có commit nền (main, có origin) — nhưng mọi thay đổi của các phiên gần đây (AGENTS.md, CLAUDE.md, memory files, openspec mới, onboarding, TEST_ADS, cleartext plugin, bug fixes) vẫn đang là working tree chưa commit.
...
7. **Lessons check:** Gọi `GET /lessons`, `GET /crystals` (AgentMemory) — xem có bài học/lỗi cũ nào liên quan tới loại thay đổi này không, tránh lặp lại.
