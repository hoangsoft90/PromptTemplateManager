...
`react-native-google-mobile-ads` (v16) with the Expo config plugin. App IDs in `app.json` use the **production** AdMob account for Android (`ca-app-pub-6917313063209470...`, package `com.hoangweb.prompttemplate` with `google-services.json` wired via `android.googleServicesFile`). Unit IDs are selected by the `TEST_ADS` flag in `lib/config.ts` (default `true`): when true, all four formats (banner/interstitial/rewarded/app open) use Google's official **test** IDs on Android and iOS and the UMP consent gate is skipped (test ads never serve real traffic); when false, Android uses the production IDs and iOS still uses test IDs as placeholders until an iOS release is prepared. Banner (ANCHORED_ADAPTIVE) pinned to the bottom of Home, Settings and Detail; interstitial, rewarded and app-open ads each managed by a singleton that preloads in the background. All ad requests are gated on the UMP consent flow (`lib/ads.ts` `initializeAds`/`canRequestAds`) when not in test mode: the SDK and ad preloads only start once `canRequestAds` is true (`delayAppMeasurementInit` + consent proguard rule configured), and Settings shows a privacy-options re-review row when the UMP flow reports it required.

- **Interstitial**: every 15 copy actions (`app_meta['ad_copy_count']`). When the threshold is hit, the global `AdGateHost` dialog offers a choice — watch a rewarded ad to earn a 20-copy shield, or show the interstitial. The dialog lives at the root layout (module-level `adGateStore`) so it survives `router.back()` after copy.
